    ...
    public class SQLFUNC
    {
        ...
        public static bool UpdateComplaint(Complaint cmp)
        {
            using (SqlConnection cnn = new SqlConnection(strCRMARConnectionString))
            {
                ...
                cmd.Parameters.AddWithValue("@param_dob", Convert.ToDateTime(cmp.Complainant.DOB));
          ...
          "(LAST_NAME,FIRST_NAME,MIDDLE_NAME,DOB,STREET,CITY,ZIP,HOME_NUMBER,MOBILE_NUMBER,Gender_Id,AKA)" +
             " VALUES (@param_Lname,@param_fname,@param_Mname,@param_dob,@param_Street,@param_City,@param_Zip,@param_homenumber," +
                 "@param_mobilenumber,@Gender_Id,@param_aka)";
                        ...
                        cmd.Parameters.AddWithValue("@param_dob", Convert.ToDateTime(item.Person.DOB));
        ...
        public static Complaint GetComplaints(int ComplaintId)
        {
            ...
            string strQry = "select Person.Id AS [CompId],Person.Last_name,person.first_Name,person.Middle_Name," +
                ...
                "Person.AKA,Person.Gender_Id,Complaints.Id As [Id],Complaints.Report_Date,Complaints.Incident_Occurance_Date,Complaints.Narrative,Complaints.Complainant_Id," +
                "Complaints.Disposition_Id,Complaints.Be_Id,Complaints.Incident_Type_Id,Complaints.Incident_Location_Id,Complaints.Report_Reviewed_By_Id,Complaints.Report_Written_By_Id,Incident_Type.Description,Incident_Location.Description AS [Desc] " +
                "from INCIDENT_Complaints Complaints " +
                "inner join INCIDENT_Person Person on person.Id = Complaints.Complainant_Id " +
                "left outer join INCIDENT_Type Incident_Type on Complaints.Incident_Type_Id = Incident_Type.Id " +
                "left outer join Incident_Location on Complaints.Incident_Location_Id = Incident_Location.Id " +
        ...
        public static List<Complaint> GetMaingridComplaints()
        {
            ...
            SqlConnection conn = new SqlConnection(strCRMARConnectionString);
            conn.Open();
            string strQry = @"select Complaints.Id As [Id], 
                            Incident_Type.Description,
                            Incident_Location.Description AS [Desc],
                            Complaints.Report_Date,
                            Complaints.Incident_Occurance_Date,
                            Person.Last_name,
                            person.first_Name
                            from INCIDENT_Complaints Complaints
                            inner join INCIDENT_Person Person on person.Id = Complaints.Complainant_Id
